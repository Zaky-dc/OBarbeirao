  ...
  return (
    ...
    >
      ...
      <div className="p-4 md:absolute md:inset-0 md:bg-black/50 md:backdrop-blur-sm md:opacity-0 md:group-hover:opacity-100 md:transition-opacity md:duration-300 md:flex md:flex-col md:justify-end">
        <h3 className="text-lg font-semibold text-gray-800 md:text-white">{servico.nome}</h3>
        <p className="text-sm text-gray-600 md:text-white mb-2">Preço: {servico.preco} MZN</p>
        <button
          onClick={() =>
            jaExiste
              ? setCarrinho(carrinho.filter((s) => s._id !== servico._id))
              : setCarrinho([...carrinho, servico])
          }
